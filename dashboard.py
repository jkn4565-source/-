...
# ==========================================
    ...
    if st.button("🚀 플랫폼별 추천 판매가 계산", type="primary", use_container_width=True):
        ...
        for (name, fee), col in zip(
            [("스마트스토어(7%)", 0.07), ("쿠팡(11%)", 0.11), ("11번가(13%)", 0.13)],
            st.columns(3)
        ):
...
# ==========================================
    ...
    # ── 디버그: 구글 시트 원본 데이터 확인 ───────────────────────
    with st.expander("🔧 데이터 확인 (문제 발생 시 클릭)", expanded=False):
        st.write("**구글 시트에서 읽어온 원본 데이터:**")
        st.json(목록[:3] if 목록 else [])
        if 목록:
            st.write(f"총 {len(목록)}개 / 첫 번째 항목 키: {list(목록[0].keys())}")
